...
| <a name="input_nat_gateways"></a> [nat\_gateways](#input\_nat\_gateways) | n/a | <pre>map(object({<br/>    subnet = optional(string, null) # If provided, NAT gateway is zonal; if null, NAT gateway is regional<br/>    type   = optional(string, "public")<br/>    az_addresses = optional(list(object({<br/>      az_id = string       # Can be a number (e.g., 1) or full AZ ID (e.g., "euw2-az1")<br/>      eips  = list(string) # List of EIP names from elastic_ips<br/>    })), [])<br/>    tags = optional(map(string), {})<br/>    eips = optional(list(string), [])<br/>  }))</pre> | `{}` | no |
